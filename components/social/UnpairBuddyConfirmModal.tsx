...
type UnpairBuddyConfirmModalProps = {
  open: boolean;
  target: UnpairBuddyTarget | null;
  busy?: boolean;
  error?: string | null;
  onConfirm: () => void;
  onCancel: () => void;
...
export function UnpairBuddyConfirmModal({
  open,
  target,
  busy = false,
  error,
  ...
  return (
    <ModalRoot>
      <AnimatePresence>
        {open && target ? (
          <motion.div
            key="unpair-buddy-confirm-overlay"
          ...
          >
            <ModalBackdrop
              label="Cancel study buddy removal"
            ...
            <motion.div
              role="dialog"
              aria-modal="true"
              aria-labelledby="unpair-buddy-dialog-title"
              aria-describedby="unpair-buddy-dialog-desc"
              ...
              <div className="p-6">
                <div className="mb-4 flex items-start gap-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-alert/12 ring-1 ring-alert/20">
                    <HeartOff className="h-5 w-5 text-alert" />
                  </div>
                  <div className="min-w-0">
                    <h2
                      id="unpair-buddy-dialog-title"
                ...
                <p
                  id="unpair-buddy-dialog-desc"
                  className="text-sm leading-relaxed text-muted"
                >
                  This will remove the study buddy relationship for both you and{" "}
                  <span className="font-medium text-text">{target.displayName}</span>.
                  Shared study buddy features, tracking, and the +20% XP bonus will
                  no longer be available until you pair with someone again.
                </p>
